...
void dialog_new_element::load_from_element(element *e)
{
    if (e) {
        ...
        case ET_ANALOG_STICK:
            ...
            m_binary_choice->select_item(static_cast<int>(stick->get_stick()));
        ...
        case ET_TRIGGER:
            ...
            m_binary_choice->select_item(static_cast<int>(trigger->get_side()));
...
{
    return static_cast<uint8_t>(SDL_strtol(m_z_level->get_text()->c_str(), nullptr, 10));
}

element_side dialog_new_element::get_side() const
{
    if (m_binary_choice) {
        return static_cast<element_side>(m_binary_choice->get_selected());
    }
    return element_side::LEFT;
}
