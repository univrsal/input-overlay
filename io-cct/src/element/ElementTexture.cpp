...
ElementTexture::ElementTexture(const element_type t, const std::string &id, const SDL_Point pos, const SDL_Rect mapping,
...
{
    return new ElementTexture(id, read_position(file, id), read_mapping(file, id, default_dim), read_layer(file, id));
}
