...
ElementTrigger::ElementTrigger(const std::string &id, const SDL_Point pos, const SDL_Rect mapping, const element_side s,
                               const trigger_direction d, const uint8_t z) : ElementTexture(TRIGGER, id, pos, mapping,
                                                                                            z)
{
...
ElementTrigger::ElementTrigger(const std::string &id, const SDL_Point pos, const SDL_Rect mapping, const element_side s,
                               const uint8_t z) : ElementTexture(TRIGGER, id, pos, mapping, z), m_direction()
{
    m_pressed_mapping = m_mapping;
    m_pressed_mapping.y += m_mapping.h + CFG_INNER_BORDER;
    m_side = s;
...
ElementTrigger* ElementTrigger::read_from_file(ccl_config* file, const std::string &id, SDL_Point* default_dim)
{
    ...
    if (button_mode) {
        return new ElementTrigger(id, read_position(file, id), read_mapping(file, id, default_dim), s,
                                  read_layer(file, id));
    }
    trigger_direction d;
    ...
    return new ElementTrigger(id, read_position(file, id), read_mapping(file, id, default_dim), s, d,
                              read_layer(file, id));
}
