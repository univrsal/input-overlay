...
void config::handle_events(SDL_Event* e)
{
    ...
    if (e->type == SDL_MOUSEBUTTONDOWN) {
        ...
        /* Selecting multiple elements */ {
            ...
            m_temp_selection.w = ceil(SDL_abs(m_selection_start.x - e->button.x) / static_cast<float>(m_cs.get_scale()));
            m_temp_selection.h = ceil(SDL_abs(m_selection_start.y - e->button.y) / static_cast<float>(m_cs.get_scale()));
...
void config::write_config(notifier* n)
{
    ...
    if (cfg.has_fatal_errors()) {
        ...
        const auto result = sdl_helper::format(m_helper->loc(LANG_MSG_SAVE_SUCCESS).c_str(), m_elements.size(), (end - start));
...
void config::read_config(notifier* n)
{
    ...
    if (cfg.has_fatal_errors()) {
        ...
        const auto result = sdl_helper::format(m_helper->loc(LANG_MSG_LOAD_SUCCESS).c_str(), m_elements.size(), (end - start));
        n->add_msg(MESSAGE_INFO, result);
    }
}
