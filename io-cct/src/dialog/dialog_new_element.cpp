...
trigger_direction dialog_new_element::get_direction() const
{
    if (m_direction) {
...
mouse_movement_type dialog_new_element::get_mouse_type() const
{
    if (m_binary_choice) {
        switch (m_binary_choice->get_selected()) {
            case DOT:
...
void dialog_new_element::handle_error(ElementError e) const
{
    switch (e) {
        case ID_EMPTY:
        case ID_NOT_UNIQUE:
