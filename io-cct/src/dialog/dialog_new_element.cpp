...
void dialog_new_element::init()
{
	...
	if (m_type == ET_ANALOG_STICK)
		add_mouse_or_analog_stick(
			LANG_LABEL_STICK_SIDE, LANG_LEFT, LANG_RIGHT);
		...
		add_mouse_or_analog_stick(
			LANG_LABEL_MOUSE_TYPE, LANG_ITEM_MOUSE_TYPE_DOT,
			LANG_ITEM_MOUSE_TYPE_ARROW);
	...
	case ET_GAMEPAD_ID:
	case ET_DPAD_STICK:
		add(m_selector = new atlas_selector(
			    m_id++, get_left() + 270, get_top() + 30,
			    m_dimensions.w - 278,
	...
	add(m_ok = new button(ACTION_OK, 8, m_dimensions.h - 32, LANG_BUTTON_OK,
	                      this));
	add(m_cancel = new button(ACTION_CANCEL, 124, m_dimensions.h - 32,
	                          LANG_BUTTON_CANCEL, this));
}
...
void dialog_new_element::action_performed(const int8_t action_id)
{
	...
	switch (action_id) {
	case ACTION_OK:
		error = m_tool->verify_element(
			this, m_modify_mode && name_unmodified);
		if (error == EE_VALID) {
			if (m_modify_mode)
				m_tool->action_performed(
					TOOL_ACTION_MOD_ELEMENT_APPLY);
			else
				m_tool->action_performed(
					TOOL_ACTION_NEW_ELEMENT_ADD);
		} else {
			handle_error(error);
		}
		break;
	...
	case ACTION_TEXT_TYPED:
		if (m_selector && !m_selector->selection_changing() && m_u &&
		    m_v && m_w && m_h) {
...
uint16_t dialog_new_element::get_vc() const
{
	...
	if (m_keycode) {
		t = static_cast<uint16_t>(strtoul(
			m_keycode->get_text()->c_str(), nullptr, 16));
	}
...
void dialog_new_element::add_selection_elements()
{
	...
	add(new label(m_id++, 8, m_element_y, LANG_LABEL_TEXTURE_SELECTION,
	              this));

	m_element_y += 25;
	add(new label(m_id++, 8, m_element_y, LANG_LABEL_WIDTH, this));
	add(new label(m_id++, 16 + panel_w / 2, m_element_y, LANG_LABEL_HEIGHT,
	              this));
	add(m_w = new textbox(m_id++, 8 + panel_w / 4 + 4, m_element_y,
	                      panel_w / 4, 20, "0", this));
	add(m_h = new textbox(m_id++, 8 + panel_w / 4 * 3 + 4, m_element_y,
	                      panel_w / 4, 20, "0", this));

	m_element_y += 25;
	add(new label(m_id++, 8, m_element_y, LANG_LABEL_U, this));
	add(new label(m_id++, 16 + panel_w / 2, m_element_y, LANG_LABEL_V,
	              this));
	add(m_u = new textbox(m_id++, 8 + panel_w / 4 + 4, m_element_y,
	                      panel_w / 4, 20, "0", this));
	add(m_v = new textbox(m_id++, 8 + panel_w / 4 * 3 + 4, m_element_y,
	                      panel_w / 4, 20, "0", this));
...
void dialog_new_element::add_keycode_elements()
{
	...
	m_element_y += 25;
	add(m_keycode = new textbox(m_id++, 8, m_element_y, panel_w, 20, "0",
	                            this));
	m_keycode->set_cutoff(10);
	m_keycode->set_flags(TEXTBOX_HEX | TEXTBOX_KEYBIND);
	m_element_y += 25;
	add(m_read_keybind = new checkbox(m_id++, 8, m_element_y,
	                                  LANG_CHECKBOX_RECORD_KEYBIND, this,
	                                  true));
	m_element_y += 40;
}
...
void dialog_new_element::add_element_id()
{
	...
	m_element_y += 25;
	add(m_element_id = new textbox(m_id++, 8, m_element_y, panel_w, 20,
	                               ELEMENT_UNNAMED, this));
...
void dialog_new_element::add_z_level()
{
	...
	m_element_y += 25;
	add(m_z_level = new textbox(m_id++, 8, m_element_y, panel_w, 20, "1",
	                            this));
	m_z_level->set_flags(TEXTBOX_NUMERIC);
	m_element_y += 40;
}

void dialog_new_element::add_mouse_or_analog_stick(
	const char *label_text, const char *item_a, const char *item_b)
{
	if (m_element_y == 0)
		m_element_y = 30;
	add(new label(m_id++, 9, m_element_y, label_text, this));
	m_element_y += 25;
	add(m_binary_choice = new combobox(m_id++, 8, m_element_y, panel_w, 20,
	                                   this));
	m_binary_choice->add_item(item_a);
	m_binary_choice->add_item(item_b);
	m_element_y += 25;
	add(new label(m_id++, 9, m_element_y, LANG_LABEL_MOVEMENT_RADIUS,
	              this));
	m_element_y += 25;
	add(m_radius = new textbox(m_id++, 8, m_element_y, panel_w, 20, "0",
	                           this));
	m_z_level->set_flags(TEXTBOX_NUMERIC);
...
void dialog_new_element::add_trigger()
{
	...
	add(new label(m_id++, 9, m_element_y, LANG_LABEL_TRIGGER_DIRECTION,
	              this));
	m_element_y += 25;
	add(m_direction = new combobox(m_id++, 8, m_element_y, panel_w, 20,
	                               this));
	...
	add(m_trigger_mode = new checkbox(m_id++, 8, m_element_y,
	                                  LANG_CHECKBOX_TRIGGER_BUTON, this));
	m_element_y += 40;
}
