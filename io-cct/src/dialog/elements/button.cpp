...
button::button(const int8_t id, const int x, const int y, const char *text,
               dialog *parent)
	: button(id, x, y, 100,
	         text, parent)
{
...
button::button(const int8_t id, const int x, const int y, const int w,
               const char *text, dialog *parent)
{
...
void button::resize()
{
	const auto text_dim = m_parent_dialog->helper()->util_text_dim(
		&m_localized_text, m_font);

	m_dimensions.h = UTIL_MAX(text_dim.h + 4, m_dimensions.h);
	m_dimensions.w = m_dimensions.w > text_dim.w + 10
		                 ? m_dimensions.w
		                 : text_dim.w + 10;
...
void button::draw_background()
{
	...
	if (m_pressed) {
		...
		get_helper()->util_text(&m_localized_text,
		                        get_dimensions()->x + m_text_pos.x,
...
bool button::handle_events(SDL_Event *event, bool was_handled)
{
	...
	if (!was_handled) {
		...
		} else if (event->type == SDL_MOUSEBUTTONUP && event->button.button ==
		           SDL_BUTTON_LEFT) {
	...
	if (event->type == SDL_KEYDOWN && m_tab_focused && event->key.keysym.sym ==
	    SDLK_RETURN) {
		m_pressed = true;
		handled = true;
