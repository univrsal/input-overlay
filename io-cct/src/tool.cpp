...
tool::tool(sdl_helper* helper, const char* texture, const char* config)
    : m_event()
{
    m_helper = helper;
    m_state = IN_SETUP;
...
void tool::handle_input()
{
    ...
    if (m_queued_dialog != NONE) {
        ...
        case MOD_ELEMENT:
            if (m_config->selected()) {
                m_state = IN_NEW_ELEMENT;
                m_toplevel = new dialog_new_element(m_helper, LANG_DIALOG_NEW_ELEMENT, this, m_config->selected()->get_type());
                m_toplevel->init();
                d = dynamic_cast<dialog_new_element*>(m_toplevel);
                d->set_default_dim(m_config->get_default_dim().x, m_config->get_default_dim().y);
