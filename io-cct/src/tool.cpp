...
ElementError tool::verify_element(dialog_new_element* d, const bool modify_mode) const
{
    /*
    ...
    if (m_queued_dialog != NONE) {
        ...
        switch (m_queued_dialog) {
            ...
            case MOD_ELEMENT:
                if (m_config->selected()) {
                    m_state = IN_NEW_ELEMENT;
                    m_toplevel = new dialog_new_element(m_helper, LANG_DIALOG_NEW_ELEMENT, this,
