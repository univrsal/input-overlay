...
static inline pair<string, string> create_pair(NSString *owner, NSString *name,
                                               bool &ok) {
  ok = false;
  if (!owner.length || !name.length)
    return pair<string, string>("n/a", "n/a");
  string o = string([owner UTF8String],
                    [owner lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
  string n = string([name UTF8String],
