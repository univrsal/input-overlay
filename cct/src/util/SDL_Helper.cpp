...
void SDL_Helper::util_set_color(uint8_t r, uint8_t g, uint8_t b, uint8_t a) const
{
...
void SDL_Helper::util_draw_line(int x1, int y1, int x2, int y2, const SDL_Color * color) const
{
...
void SDL_Helper::util_draw_rect(int x, int y, int w, int h, const SDL_Color * color) const
{
...
void SDL_Helper::util_fill_rect_shadow(const SDL_Rect * rect, const SDL_Color * color, int8_t offset) const
{
...
void SDL_Helper::util_fill_rect(int x, int y, int w, int h, const SDL_Color * color) const
{
    util_set_color(color);
...
void SDL_Helper::util_fill_rect(const SDL_Rect * rect, const SDL_Color * color, uint8_t alpha) const
{
    SDL_Color temp;
    temp.a = alpha;
    temp.r = color->r;
...
bool SDL_Helper::util_is_in_rect(const SDL_Rect * rect, int x, int y) const
{
    return x >= rect->x && x <= (rect->x + rect->w) && y >= rect->y && y <= (rect->y + rect->h);
}

void SDL_Helper::util_text(const std::string * text, int x, int y, const SDL_Color * color, uint8_t font) const
{
...
void SDL_Helper::util_text_rot(const std::string * text, int x, int y, const SDL_Color * color, double angle, uint8_t font) const
{
    TTF_Font * ttf_font = get_font(font);
    if (color == nullptr)
...
SDL_Rect SDL_Helper::util_text_dim(const std::string * text, uint8_t font) const
{
...
void SDL_Helper::util_cut_string(std::string & s, int max_width, bool front) const
{
...
void SDL_Helper::set_cursor(uint8_t type) const
{
    if (m_have_cursors)
    {
        switch (type)
...
bool SDL_Helper::is_ctrl_down(void) const
{
    return m_ctrl_down;
}

bool SDL_Helper::is_shift_down(void) const
{
    return m_shift_down;
}

void SDL_Helper::exit_loop(void) const
{
    *m_runflag = false;
}
...
uint8_t SDL_Helper::util_font_height(uint8_t font) const
{
    switch (font)
    {
        default:
...
void SDL_Helper::cap_frame(void) const
{
    if (m_frame_cap_timer.get_delta() < SDL_WINDOW_TPF)
        SDL_Delay(SDL_WINDOW_TPF - m_frame_cap_timer.get_time());
}

float SDL_Helper::util_get_fps(void) const
{
    return m_fps;
}

std::wstring SDL_Helper::util_utf8_to_wstring(const std::string& str) const
{
#ifdef WINDOWS
...
std::string SDL_Helper::util_wstring_to_utf8(const std::wstring& str) const
{
#ifdef WINDOWS
    std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
...
void SDL_Helper::format_text(const std::string * s, std::vector<std::unique_ptr<std::string>> & out, SDL_Rect & dim, uint8_t font) const
{
    if (!s || s->empty())
        return;
...
static uint32_t KEY_MAP[][2]
{
    ...
    /* Numbers */
    { VC_0, SDLK_0 },{ VC_1, SDLK_1 },{ VC_2, SDLK_2 },{ VC_3, SDLK_3 },{ VC_4, SDLK_4 },{ VC_5, SDLK_5 },{ VC_6, SDLK_6 },{ VC_7, SDLK_7 },
    { VC_8, SDLK_8 },{ VC_9, SDLK_9 },{ VC_KP_0, SDLK_KP_0 },{ VC_KP_1, SDLK_KP_1 },{ VC_KP_2, SDLK_KP_2 },{ VC_KP_3, SDLK_KP_3 },
    { VC_KP_4, SDLK_KP_4 },{ VC_KP_5, SDLK_KP_5 },{ VC_KP_6, SDLK_KP_6 },{ VC_KP_7, SDLK_KP_7 },{ VC_KP_8, SDLK_KP_8 },{ VC_KP_9, SDLK_KP_9 },
    { VC_KP_ADD, SDLK_KP_PLUS}, { VC_KP_SEPARATOR, SDLK_KP_COMMA},

    /* Numpad misc */
    { VC_KP_ADD, SDLK_KP_PLUS },{ VC_KP_SUBTRACT, SDLK_KP_MINUS },{ VC_KP_ENTER, SDLK_KP_ENTER },{ VC_KP_MULTIPLY, SDLK_KP_MULTIPLY },
    ...
    /* Mask keys*/
    { VC_ALT_L, SDLK_LALT },{ VC_ALT_R, SDLK_RALT },{ VC_CONTROL_L, SDLK_LCTRL },{ VC_CONTROL_R, SDLK_RCTRL },{ VC_SHIFT_L, SDLK_LSHIFT },
    ...
    { VC_DELETE, SDLK_DELETE },{ VC_INSERT, SDLK_INSERT },{ VC_HOME, SDLK_HOME },{ VC_PAGE_UP, SDLK_PAGEUP },{ VC_PAGE_DOWN, SDLK_PAGEDOWN },
    { VC_END, SDLK_END },
    ...
    { PAD_TO_VC(PAD_LB), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_LEFTSHOULDER) },{ PAD_TO_VC(PAD_RB), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_RIGHTSHOULDER) },
    { PAD_TO_VC(PAD_BACK), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_BACK) },{ PAD_TO_VC(PAD_START), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_START) },
    { PAD_TO_VC(PAD_DPAD_DOWN), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_DPAD_DOWN) },{ PAD_TO_VC(PAD_DPAD_UP), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_DPAD_UP) },
    { PAD_TO_VC(PAD_DPAD_LEFT), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_DPAD_LEFT) },{ PAD_TO_VC(PAD_DPAD_RIGHT), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_DPAD_RIGHT) },
    { PAD_TO_VC(PAD_L_ANALOG), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_LEFTSTICK) },{ PAD_TO_VC(PAD_R_ANALOG), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_RIGHTSTICK) },
    { PAD_TO_VC(PAD_X_BOX_KEY), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_GUIDE) },{ PAD_TO_VC(PAD_START), TO_PAD_MASK(SDL_CONTROLLER_BUTTON_START) },
...
uint32_t SDL_Helper::vc_to_sdl_key(uint16_t key) const
{
    for (auto & i : KEY_MAP)
    {
        if (i[0] == key)
...
uint16_t SDL_Helper::sdl_key_to_vc(uint32_t key) const
{
    for (auto & i : KEY_MAP)
    {
        if (i[1] == key)
