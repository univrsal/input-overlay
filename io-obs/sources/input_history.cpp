...
bool toggle_font_settings(obs_properties_t* props, obs_property_t* p, obs_data_t* data)
{
    ...
    for (; prop; obs_property_next(&prop)) {
        ...
        if (!strcmp(name, "read_from_file") || !strcmp(name, "text") || !strcmp(name, "file") || !strcmp(name, "vertical") || strstr(name, "align") || strstr(name, "extents") || strstr(name, "chatlog") || (!use_gradient && strstr(name, "gradient") && strlen(name) > strlen("gradient")) || (!use_outline && strstr(name, "outline") && strlen(name) > strlen("outline"))) { /* do not show unnecessary properties */
...
obs_properties_t* get_properties_for_history(void* data)
{
    ...
    const auto mode_list = obs_properties_add_list(props, S_HISTORY_MODE, T_HISTORY_MODE, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    ...
    obs_properties_add_float_slider(props, S_HISTORY_AUTO_CLEAR_INTERVAL, T_HISTORY_AUTO_CLEAR_INTERVAL, 0.1, 10, 0.1);
...
void register_history()
{
    ...
    si.get_width = [](void* data) { return reinterpret_cast<input_history_source*>(data)->get_width(); };
    si.get_height = [](void* data) { return reinterpret_cast<input_history_source*>(data)->get_height(); };
