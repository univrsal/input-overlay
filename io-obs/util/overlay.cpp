...
bool overlay::load_cfg()
{
    ...
    if (err.error == QJsonParseError::NoError) {
        ...
        berr("Couldn't load layout from %s. Error: %s", m_settings->layout_file.c_str(),
...
void overlay::load_element(const QJsonObject& obj, const bool debug)
{
    ...
    default:
        if (debug) {
            binfo("Invalid element type %i for %s", type,
                qt_to_utf8(obj[CFG_ID].toString()));
        }
    }
    ...
    if (new_element) {
        ...
        {
#endif
            binfo("Type: %14s, KEYCODE: 0x%04X ID: %s",
                element_type_to_string(static_cast<element_type>(type)),
