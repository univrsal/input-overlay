...
static bool get_window_title(const HWND window, std::string &title)
{
    auto len = static_cast<size_t>(GetWindowTextLengthW(window));
    std::wstring wtitle;

    wtitle.resize(len);
    if (!GetWindowTextW(window, &wtitle[0], static_cast<int>(len) + 1))
        return false;
...
static bool window_valid(const HWND window)
{
    RECT rect;
    DWORD id;
    ...
    const auto styles = GetWindowLongPtr(window, GWL_STYLE);
    const auto ex_styles = GetWindowLongPtr(window, GWL_EXSTYLE);
...
{
    auto window = GetWindow(GetDesktopWindow(), GW_CHILD);

    while (window) {
        std::string title;
        if (window_valid(window) && get_window_title(window, title))
...
static std::vector<Window> get_toplevel_windows()
{
...
static std::string get_window_title(size_t i)
{
...
void get_window_list(std::vector<std::string> &windows)
{
    ...
    for (size_t i = 0; i < getTopLevelWindows().size(); ++i) {
        if (GetWindowTitle(i) != "")
            windows.emplace_back(get_window_title(i));
    }
}
