...
element_data_analog_stick* element_data_analog_stick::from_buffer(netlib_byte_buf* buffer)
{
    ...
    if (!netlib_read_float(buffer, &result->m_left_stick.x) || !netlib_read_float(buffer, &result->m_left_stick.y) || !netlib_read_float(buffer, &result->m_right_stick.x) || !netlib_read_float(buffer, &result->m_right_stick.y)) {
#ifdef _DEBUG
