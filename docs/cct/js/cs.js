var cs = {
    ...
    draw(painter) {
        // Axis labels
        painter.text("x in pixels", this.origin.x - 17, this.origin.y - 20, 90, "right", lineColor, 15);
        painter.text("y in pixels", this.origin.x - 22, this.origin.y - 9, 0, "right", lineColor, 15);
        // Draw axis
        ...
        for (var x = this.origin.x - this.offset.x; x < this.dimensions.w + this.origin.x; x += this.step * this.scale) {
        ...
        counter = 0;
        for (var y = this.origin.y - this.offset.y; y < this.dimensions.h + this.origin.y; y += this.step * this.scale) {
            let div = counter % this.step === 0;
            let size = div ? 5.5 : 2.5;
    ...
    move(event) {
        if (this.dragging) {
            this.offset = this.orig_offset.add(this.drag_start.sub(event.clientX, event.clientY));
            this.offset.max();
        }
    },

    resize(painter) {
        this.dimensions = r2($('#main-canvas').attr("width") - this.origin.x, $('#main-canvas').attr("height") - this.origin.y);
    }
};