  ...
  draw(painter) {
    ...
    if (pad.lastInput) {
      ...
      painter.image_crop(
        ...
        this.w() * cs.scale,
        this.h() * cs.scale,
        this.u() + (constants.texture_space + this.w()) * pad.lastInput.index,
        this.v(),
        this.w(),
        this.h()
      );
      ...
      if (this.data.trigger_mode) {
        ...
        if (this.data.direction === 1) {
          ...
          painter.image_crop(
            atlas,
            cs.origin.x - cs.offset.x + this.x() * cs.scale,
            cs.origin.y - cs.offset.y + (this.y() + offset) * cs.scale,
            this.w() * cs.scale,
          ...
          painter.image_crop(
            atlas,
            cs.origin.x - cs.offset.x + (this.x() + offset) * cs.scale,
            cs.origin.y - cs.offset.y + this.y() * cs.scale,
...
element_map.set(element_types.MOUSE_MOVEMENT, (json) => {
  return new mouse_movement(json);
});
