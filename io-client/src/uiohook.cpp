...
bool data_holder::write_to_buffer(netlib_byte_buf* buffer)
{
    ...
    if (m_new_mouse_data) {
        success = netlib_write_uint8(buffer, MSG_MOUSE_DATA) && netlib_write_int16(buffer, m_mouse_x) && netlib_write_int16(buffer, m_mouse_y) && netlib_write_int8(buffer, m_wheel_direction) && netlib_write_int16(buffer, m_wheel_amount) && netlib_write_int8(buffer, m_wheel_pressed);
