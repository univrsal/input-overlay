...
int write_gamepad_data()
{
    ...
    for (auto& pad : gamepad::pads) {
        if (pad.changed()) {
            if (!netlib_write_uint8(network::buffer, pad.get_id()) || !netlib_write_uint16(network::buffer, pad.get_state()->button_states) || !netlib_write_float(network::buffer, pad.get_state()->stick_l_x) || !netlib_write_float(network::buffer, pad.get_state()->stick_l_y) || !netlib_write_float(network::buffer, pad.get_state()->stick_r_x) || !netlib_write_float(network::buffer, pad.get_state()->stick_r_y) || !netlib_write_uint8(network::buffer, pad.get_state()->trigger_l) || !netlib_write_uint8(network::buffer, pad.get_state()->trigger_r))
                result = 0;
