...
void InputSource::draw_key(gs_effect_t* effect, InputKey* key, uint16_t x, uint16_t y, bool rot, float angle)
{
    ...
    if (rot) {
        ...
        if (key->m_pressed) {
            gs_draw_sprite_subregion(m_image->texture, 0, key->texture_u, key->texture_v + key->h + 3, key->w + 1,
                                     key->h + 1);
        } else {
            gs_draw_sprite_subregion(m_image->texture, 0, key->texture_u, key->texture_v, key->w + 1, key->h + 1);
        }
    ...
    if (m_layout_file.empty() || !m_layout.m_is_loaded) {
        ...
        if (m_layout.m_type == TYPE_KEYBOARD) {
            ...
            if (m_gamepad) {
                ...
                x = (uint16_t) (k->column - k->w / 2 + m_layout.m_track_radius * m_gamepad->left_stick_x());
                y = (uint16_t) (k->row - k->h / 2 - m_layout.m_track_radius * m_gamepad->left_stick_y());

                draw_key(effect, k, x, y);

                k = &m_layout.m_keys[PAD_R_ANALOG];
                x = (uint16_t) (k->column - k->w / 2 + m_layout.m_track_radius * m_gamepad->right_stick_x());
                y = (uint16_t) (k->row - k->h / 2 - m_layout.m_track_radius * m_gamepad->right_stick_y());
                draw_key(effect, k, x, y);
...
void InputSource::load_layout()
{
        ...
        if (m_layout.m_type == TYPE_KEYBOARD) {
            ...
            for (int i = 0; i < m_layout.m_key_count; i++) {
                ...
                if (tempw > 1 && m_layout.m_key_space_h > 0) {
                    k.x_offset = (uint16_t) ((k.w + m_layout.m_key_space_h * (tempw - 1)) / 2 - k.w / 2);
                    k.w -= k.x_offset;
                    index += tempw;
                } else {
                    index++;
            ...
            std::string vals[] = {"lmb", "rmb", "mmb", "smb1", "smb2", "mwu", "mwd", "body", "arrow", "field", "dot"};
            uint16_t keys[]{VC_MOUSE_BUTTON1, VC_MOUSE_BUTTON2, VC_MOUSE_BUTTON3, VC_MOUSE_BUTTON5, VC_MOUSE_BUTTON4,
                            VC_MOUSE_WHEEL_UP, VC_MOUSE_WHEEL_DOWN, VC_NONE, VC_NONE, VC_NONE, VC_NONE};

            std::stringstream stream;
            std::string begin;

            int max = (m_layout.m_mouse_movement && !m_layout.m_use_arrow) ? m_layout.m_key_count + 1
                                                                           : m_layout.m_key_count;
            ...
            keys[PAD_L_ANALOG].h = keys[PAD_L_ANALOG].w = keys[PAD_R_ANALOG].h = keys[PAD_R_ANALOG].w = (uint16_t) cfg->get_int(
                    "pad_analog_dim");
            ...
            keys[PAD_PLAYER_1].column = keys[PAD_PLAYER_2].column = keys[PAD_PLAYER_3].column = keys[PAD_PLAYER_4].column = (uint16_t) cfg->get_int(
                    "pad_port_x");
            keys[PAD_PLAYER_1].row = keys[PAD_PLAYER_2].row = keys[PAD_PLAYER_3].row = keys[PAD_PLAYER_4].row = (uint16_t) cfg->get_int(
                    "pad_port_y");

            keys[PAD_PLAYER_1].w = keys[PAD_PLAYER_2].w = keys[PAD_PLAYER_3].w = keys[PAD_PLAYER_4].w = keys[PAD_PLAYER_1].h = keys[PAD_PLAYER_2].h = keys[PAD_PLAYER_3].h = keys[PAD_PLAYER_4].h = (uint16_t) cfg->get_int(
                    "pad_port_dim");

            keys[PAD_PLAYER_1].texture_u = (uint16_t) cfg->get_int("pad_port_u");
            keys[PAD_PLAYER_1].texture_v = keys[PAD_PLAYER_2].texture_v = keys[PAD_PLAYER_3].texture_v = keys[PAD_PLAYER_4].texture_v = (uint16_t) cfg->get_int(
                    "pad_port_v");
            ...
            // X, Y, A, B
            keys[PAD_A].texture_v = keys[PAD_B].texture_v = keys[PAD_Y].texture_v = keys[PAD_X].texture_v = (uint16_t) cfg->get_int(
                    "pad_x_v");
            keys[PAD_X].texture_u = (uint16_t) cfg->get_int("pad_x_u");

            keys[PAD_X].w = keys[PAD_Y].w = keys[PAD_A].w = keys[PAD_B].w = keys[PAD_X].h = keys[PAD_Y].h = keys[PAD_A].h = keys[PAD_B].h = (uint16_t) cfg->get_int(
                    "pad_x_dim");
            ...
            // Dpad
            keys[PAD_DPAD_UP].texture_u = keys[PAD_DPAD_DOWN].texture_u = keys[PAD_DPAD_LEFT].texture_u = keys[PAD_DPAD_RIGHT].texture_u = (uint16_t) cfg->get_int(
                    "pad_dpad_pressed_u");
            keys[PAD_DPAD_UP].texture_v = keys[PAD_DPAD_DOWN].texture_v = keys[PAD_DPAD_LEFT].texture_v = keys[PAD_DPAD_RIGHT].texture_v = (uint16_t) cfg->get_int(
                    "pad_dpad_pressed_v");

            keys[PAD_DPAD_UP].w = keys[PAD_DPAD_DOWN].w = keys[PAD_DPAD_LEFT].w = keys[PAD_DPAD_RIGHT].w = keys[PAD_DPAD_UP].h = keys[PAD_DPAD_DOWN].h = keys[PAD_DPAD_LEFT].h = keys[PAD_DPAD_RIGHT].h = (uint16_t) cfg->get_int(
                    "pad_dpad_dim");
...
#endif
    {
        ...
        for (int i = 0; i < m_layout.m_key_count; i++) {
            blog(LOG_INFO,
                 "KEY ID: %02d, CODE: %#04X, X: %05d, Y: %05d, U: %05d, V: %05d, W: %05d, H: %05d, X_OFF: %05d", i,
                 m_layout.m_keys[i].m_key_code, m_layout.m_keys[i].column, m_layout.m_keys[i].row,
                 m_layout.m_keys[i].texture_u, m_layout.m_keys[i].texture_v, m_layout.m_keys[i].w, m_layout.m_keys[i].h,
...
obs_properties_t* get_properties_for_overlay(void* data)
{
    ...
    obs_properties_add_path(props, S_OVERLAY_FILE, T_OVERLAY_FILE, OBS_PATH_FILE, filter_img.c_str(), img_path.c_str());

    obs_properties_add_path(props, S_LAYOUT_FILE, T_LAYOUT_FILE, OBS_PATH_FILE, filter_text.c_str(),
                            layout_path.c_str());
...
void register_overlay_source()
{
    ...
    { delete reinterpret_cast<InputSource*>(data); };
    si.get_width = [](void* data)
    { return reinterpret_cast<InputSource*>(data)->cx; };
    si.get_height = [](void* data)
    { return reinterpret_cast<InputSource*>(data)->cy; };
